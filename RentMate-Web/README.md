//NABOR SLIK ZA ITEM NA RENTALS INDEX IN ITEMS DETAILS STRANEH (swipable), IN-HOUSE CHAT SYSTEM, ITEM ERP WEIGHT SYSTEM + OWNER REP COMPLETION, ADMIN LAHKO NADZIRA WEIGHT, ONLY ONE ANON REVIEW PER DEVICE (OR OTHER WAY TO PREVENT ANON ABUSES)
// MOCK PAYING SYSTEM (ideally API) SEVEDA KAVCIJA NE JAMSTVO, MODERNIZACIJA SISTEMA JEZIKOV (JSON), EUR & USD BACKEND IN PRILAGOJEN PRIKAZ ZA DRUGE VALUTE (PRESONALIZED CONVERSION RATE), "ARE YOU SURE DA BI SI SPOSODU/ODDOU/SPREJEU ETC." PROMPT SPOSOJO-RENT OUTU AKA. VSE KAR JE IREVERSIBLE RABI BIT 2F POTRJENO, DODATI SECURITY features (server side)
